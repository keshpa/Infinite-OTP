...
void ENCRYPTION(vector<uint16_t>& input, vector<uint16_t>& userKey) {
	//chrono::duration<double, nano> duration;
	...
	{
		//Timer timer1("Time taken for replacement: ",  duration);
		REPLACEMENT_PORTION_FORWARD_ALL(d);
	}
	{
		//Timer timer1("Time taken for xor with OTP&EPUK: ",  duration);
		XOR_OTP_EPUK_PORTION_ALL(0);
	}
	{
		//Timer timer1("Time taken for xor with OTP&EPUkand replacement : ",  duration);
		REPLACEMENT_PORTION_FORWARD_ALL(d);
		XOR_OTP_EPUK_PORTION_ALL(1);
	}

	/* STEP 4: Put entire user data through 1 round of rearrangement */
	{
		//Timer timer1("Time taken for rearrangement : ",  duration);
		REARRANGE_ALL_PORTIONS(d);
	}

	/* STEP 5: Put entire user data through 2 round of replacement */
	{
		//Timer timer1("Time taken for xor with OTP&EPUkand replacement : ",  duration);
		REPLACEMENT_PORTION_FORWARD_ALL(d);
	...
	{
		//Timer timer1("Time taken for super rearrangement : ",  duration);
		SUPER_REARRANGEMENT_PORTION_ALL();
	}

	/* STEP 7: Put entire user data through 2 round of replacement */
	{
		//Timer timer1("Time taken for xor with OTP&EPUkand replacement : ",  duration);
		REPLACEMENT_PORTION_FORWARD_ALL(d);
	...
	{
		//Timer timer1("Time taken for storing data : ",  duration);
		SET_PORTION_ALL(d, input.data());
	}	
	//cout << "Time taken for encryption: " << convertDurationToString(duration) << endl;
}

/**** Main function to trigger decryption ****/
void DECRYPTION(vector<uint16_t>& input, vector<uint16_t>& userKey) {
	/* STEP 1: Load user key (1024-bits) and data (4K bytes). */
	LOAD_PORTION_ALL(d, input.data());
	LOAD_PORTION(k, userKey.data(), 0);
	...
	/* STEP 5: Put entire user data through 2 round of replacement (5) */
	XOR_OTP_EPUK_PORTION_ALL(3);
	REPLACEMENT_PORTION_REVERSE_ALL(d);
	XOR_OTP_EPUK_PORTION_ALL(2);
	REPLACEMENT_PORTION_REVERSE_ALL(d);

	/* STEP 6: Put entire user data through 1 round of rearrangement (4) */
	REARRANGE_ALL_PORTIONS(d);

	/* STEP 7: Put entire user data through 2 round of replacement (3) */
	XOR_OTP_EPUK_PORTION_ALL(1);
	REPLACEMENT_PORTION_REVERSE_ALL(d);
	XOR_OTP_EPUK_PORTION_ALL(0);
	REPLACEMENT_PORTION_REVERSE_ALL(d);
...
int main() {
	...
	chrono::duration<double, nano> duration;
	{
		Timer timer1("Time taken for 10,000 encryption:", duration);
	...
	assert(memcmp(reinterpret_cast<const char*>(input.data()), reinterpret_cast<const char*>(original.data()), size) != 0);
	{
		Timer timer1("Time taken for 10,000 decryption:", duration);
		for (int i = 0; i < 10000; ++i) {
